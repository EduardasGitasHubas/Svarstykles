...
write_log() {
  ...
  python - "$LOG_FILE" "$STARTED_AT" "$status" "$step_num" "$step_name" "$STEPS_COMPLETED" "$last_error" "$OLD_VERSION" <<'PY'
import json
import sys
...
render_template_if_missing() {
  ...
  python - "$template_path" "$target_path" "$ROOT_DIR" <<'PY'
import json
import subprocess
import sys
...
generate_upgrade_memory_files() {
  ...
  python - "$ROOT_DIR" "$OLD_VERSION" <<'PY'
import os
import re
import sys
import json
...
MODE_AND_VERSION="$(python - "$CONTEXT_FILE" <<'PY'
import json
import sys
from pathlib import Path
