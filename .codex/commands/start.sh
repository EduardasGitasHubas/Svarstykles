...
run_cold_start() {
  set +e
  OUTPUT="$(FRAMEWORK_AGENT_NAME=codex FRAMEWORK_OWNER_PID=$OWNER_PID python src/framework-core/main.py cold-start 2>&1)"
  EXIT_CODE=$?
  set -e
}
...
if [ -f ".claude/migration-context.json" ]; then
  ...
  NEXT_COMMAND="$(python -c 'import json,sys; print(json.loads(sys.argv[1]).get("next_command", ""))' "$ROUTE_JSON")"
...
if [ "$EXIT_CODE" -eq 0 ] && [ "${CODEX_AUTO_UPDATE:-1}" != "0" ] && [ -f ".codex/commands/quick-update.sh" ]; then
  UPDATE_RESULT="$(python .codex/utils/parse-update-result.py "$OUTPUT")"
