...
if [ -z "$CURRENT_VERSION" ]; then
    echo -e "${YELLOW}[WARN]${NC} Framework not found in this project"
    echo ""
    ...
    echo ""
    echo -e "${BLUE}[INFO]${NC} Downloading init-project.sh..."
    ...
    if ! curl -L -f "$INSTALLER_URL" -o "$INSTALLER_PATH" 2>/dev/null; then
        echo -e "${RED}[X]${NC} Failed to download init-project.sh"
        echo ""
    ...
    echo -e "${GREEN}[OK]${NC} Downloaded init-project.sh"
    echo ""
    ...
    echo ""
    echo -e "${GREEN}[OK]${NC} Framework installed successfully!"
    echo ""
    exit 0
fi

echo -e "${BLUE}[INFO]${NC} Current version: ${BOLD}${CURRENT_VERSION}${NC}"
...
if ! command -v curl &> /dev/null; then
    echo -e "${RED}[X]${NC} curl not found. Please install curl."
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo -e "${RED}[X]${NC} Failed to fetch latest version from GitHub"
    exit 1
fi
...
if [ -z "$LATEST_VERSION" ]; then
    echo -e "${RED}[X]${NC} Failed to parse latest version"
    exit 1
fi

echo -e "${BLUE}[INFO]${NC} Latest version:  ${BOLD}${LATEST_VERSION}${NC}"
echo ""
...
if [ "$CURRENT_VERSION" = "$LATEST_VERSION" ]; then
    echo -e "${GREEN}[OK]${NC} Framework is already up to date!"
    echo ""
    exit 0
fi

echo -e "${YELLOW}[WARN]${NC} Update available: ${BOLD}v${CURRENT_VERSION}${NC} -> ${BOLD}v${LATEST_VERSION}${NC}"
echo ""
...
echo -e "${BLUE}[INFO]${NC} Downloading framework update..."
...
if ! curl -L -f "$COMMANDS_URL" -o "$TEMP_DIR/framework-commands.tar.gz" 2>/dev/null; then
    echo -e "${YELLOW}[WARN]${NC} Commands archive not found, downloading full framework..."
    ...
    if ! curl -L -f "$FRAMEWORK_URL" -o "$TEMP_DIR/framework.tar.gz" 2>/dev/null; then
        echo -e "${RED}[X]${NC} Failed to download framework"
        exit 1
    fi
...
echo -e "${GREEN}[OK]${NC} Downloaded v${LATEST_VERSION}"

# ============================================
# 5. Download Latest CLAUDE.md
# ============================================

echo -e "${BLUE}[INFO]${NC} Downloading latest CLAUDE.md..."
...
if ! curl -L -f "$CLAUDE_MD_URL" -o "$TEMP_DIR/CLAUDE.md" 2>/dev/null; then
    echo -e "${YELLOW}[WARN]${NC} CLAUDE.md not found in release, skipping"
else
    echo -e "${GREEN}[OK]${NC} Downloaded CLAUDE.md"
fi
...
echo -e "${BLUE}[INFO]${NC} Applying update..."
...
if [ -d "$FRAMEWORK_DIR" ]; then
    ...
    for cmd in "$COMMANDS_SOURCE"/*.md; do
        if [ -f "$cmd" ]; then
            ...
            echo -e "${GREEN}  [OK]${NC} Updated: .claude/commands/${filename}"
        fi
    done
fi
...
if [ -f "$TEMP_DIR/CLAUDE.md" ]; then
    ...
    echo -e "${GREEN}  [OK]${NC} Updated: CLAUDE.md"
fi
...
if [ -n "$CODEX_SOURCE" ]; then
    if [ -f "$CODEX_SOURCE/AGENTS.md" ]; then
        ...
        echo -e "${GREEN}  [OK]${NC} Updated: AGENTS.md"
    fi
    ...
    if [ -d "$CODEX_SOURCE/.codex" ]; then
        ...
        echo -e "${GREEN}  [OK]${NC} Updated: .codex/"
    fi
fi
...
if [ -n "$CLAUDE_SOURCE" ] && [ -f "$CLAUDE_SOURCE/.claude/scripts/quick-update.sh" ]; then
    ...
    echo -e "${GREEN}  [OK]${NC} Updated: .claude/scripts/quick-update.sh"
fi
...
if [ -f ".claude/SNAPSHOT.md" ]; then
    if command -v sed &> /dev/null; then
        ...
        echo -e "${GREEN}  [OK]${NC} Updated: .claude/SNAPSHOT.md"
    fi
fi
...
echo "====================================================="
echo ""
echo -e "${GREEN}[START] Framework updated successfully!${NC}"
echo ""
echo "Next steps:"
