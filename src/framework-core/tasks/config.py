...
def _render_template(root: Path, template_name: str, replacements: dict) -> str:
    template_path = root / ".claude" / "templates" / template_name
...
def ensure_project_baseline(create_missing: bool = True):
    ...
    try:
        ...
        structure_plain_lines = [
            entry.replace("- `", "  ").replace("`", "")
            for entry in structure_lines
        ]

        snapshot_content = _render_template(root, "SNAPSHOT.template.md", {
            "PROJECT_NAME": root.name,
            "DATE": today,
            "PROJECT_VERSION": "Initial framework baseline",
            "CURRENT_BRANCH": branch,
            "PROJECT_DESCRIPTION": overview,
            "TECH_STACK": "\n".join(stack_lines),
            "PROJECT_STRUCTURE": "\n".join(structure_plain_lines),
            "PROJECT_KEY_CONCEPTS": "\n".join(heading_lines),
        })

        high_priority = task_lines[:3] or ["- [ ] Confirm the first high-priority task"]
        medium_priority = task_lines[3:6] or ["- [ ] Review medium-priority work after initial setup"]
        low_priority = task_lines[6:9] or ["- [ ] Capture lower-priority follow-up items"]

        backlog_content = _render_template(root, "BACKLOG.template.md", {
            "PROJECT_NAME": root.name,
            "DATE": today,
            "HIGH_PRIORITY_TASKS": "\n".join(high_priority),
            "MEDIUM_PRIORITY_TASKS": "\n".join(medium_priority),
            "LOW_PRIORITY_TASKS": "\n".join(low_priority),
            "FEATURE_ITEMS": "\n".join(task_lines[:2] or ["- [ ] Define first feature"]),
            "IMPROVEMENT_ITEMS": "\n".join(task_lines[2:4] or ["- [ ] Review generated state files and priorities"]),
            "BUG_ITEMS": "- [ ] No known bugs recorded yet",
            "TECH_DEBT_ITEMS": "- [ ] Continue work cycle via start/finish protocol",
            "NOTES": f"Source documents analyzed: {len(docs)}",
        })
        ...
        decisions_content = _render_template(root, "DECISIONS.template.md", {
            "PROJECT_NAME": root.name,
            "TODAY": today,
        })

        risks_content = _render_template(root, "RISKS.template.md", {
            "PROJECT_NAME": root.name,
            "TODAY": today,
        })

        product_content = _render_template(root, "PRODUCT.template.md", {
            "PROJECT_NAME": root.name,
        })

        architecture_notes_content = _render_template(root, "ARCHITECTURE_NOTES.template.md", {
            "PROJECT_NAME": root.name,
        })

        ai_context_content = _render_template(root, "AI_CONTEXT.template.md", {
            "PROJECT_NAME": root.name,
        })
        ...
        targets = {
            ...
            state_dir / "DECISIONS.md": decisions_content,
            state_dir / "RISKS.md": risks_content,
        }

        doc_targets = {
            docs_dir / "PRODUCT.md": product_content,
            docs_dir / "ARCHITECTURE_NOTES.md": architecture_notes_content,
            docs_dir / "AI_CONTEXT.md": ai_context_content,
        }
        ...
        for path, content in doc_targets.items():
            rel = path.relative_to(root).as_posix()
            if not path.exists():
                if create_missing:
                    path.write_text(content, encoding="utf-8")
