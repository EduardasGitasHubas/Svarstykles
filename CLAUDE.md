...
- `py src/framework-core/main.py cold-start` - Execute all tasks
...
- "task is done", "feature is ready", "bug is fixed", "tests are passing"
...
- Phase 3: Silent completion (show nothing or "[OK] Ready")
...
- **Result:** "[OK] Committed (hash)" or nothing at all
...
if [ -d "migration" ] && [ -f "migration/build-distribution.sh" ]; then
  echo ""
  echo "========================================"
  echo "[CHART] Framework Developer Mode Active"
  echo "========================================"
  echo ""
  ...
  if [ "$ISSUE_COUNT" -gt "0" ]; then
    echo "[WARN]  $ISSUE_COUNT bug report(s) available from host projects"
    echo ""
    echo "To analyze:"
...
1. **Show welcome message:**
   ...
   [START] Next Step:
     ...
     The framework will initialize the working session and load project context.
