...
REPO="${FRAMEWORK_REPO:-EduardasGitasHubas/Pradzia}"
...
detect_existing_project_profile() {
    ...
    python - "$config_file" <<'PY' 2>/dev/null || true
import json
import sys
...
render_template_file() {
    ...
    python - "$template_path" "$target_path" "$project_name" "$date_value" "$branch_value" "$profile" "$project_description" "$project_structure" <<'PY'
import sys
from pathlib import Path
...
if command -v curl &> /dev/null; then
    curl -L -f --ssl-no-revoke "$ARCHIVE_URL" -o "$TEMP_DIR/framework.tar.gz" || {
...
estimate_analysis_cost() {
    ...
    echo ""
    echo "============================================================"
    echo "  [CHART] Deep Analysis Cost Estimate"
    echo "============================================================"
    echo ""
...
case $PROJECT_TYPE in
    "new-project")
        log_info "[PACKAGE] Scenario: New Project"
    ...
    "legacy-migration")
        log_warning "[SEARCH] Scenario: Legacy Project (No Framework)"
        log_info "Project profile: $PROJECT_PROFILE"
        echo ""
        ...
        echo "  4. [ERROR] NOT modify your existing files"
        echo ""
    ...
    framework-upgrade:*)
        ...
        log_warning "[UPDATE] Scenario: Framework Upgrade (from $OLD_VERSION)"
        log_info "Project profile: $PROJECT_PROFILE"
        echo ""
        ...
        echo "  * [OK] Preserve ALL existing data"
        echo "  * [SAVE] Create backup before changes"
        echo ""
    ...
    "framework-current:v2.1")
        if [ -f "AGENTS.md" ] && [ -d ".codex" ]; then
            log_success "[OK] Framework v2.1+ already installed"
            echo ""
        ...
        log_warning "[MIGRATE] Framework is current, but Codex adapter is missing"
        log_info "Project profile: $PROJECT_PROFILE"
        echo ""
