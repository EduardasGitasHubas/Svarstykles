...
echo ""
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  [LOCK] Initial Security Scan${NC}"
...
echo -e "${YELLOW}[WARN]${NC}  Checking for .env files in repository..."
...
if [ -n "$ENV_FILES" ]; then
  ...
  echo -e "${RED}[ALERT] CRITICAL: .env files found in repository!${NC}"
  echo ""
  ...
  cat >> "$REPORT_FILE" <<EOF
[ALERT] CRITICAL: .env Files in Repository
========================================
...
EOF
  ...
  echo -e "${GREEN}[OK]${NC} No .env files in repository"
fi
...
echo -e "${YELLOW}[WARN]${NC}  Checking for credential files..."
...
if [ -n "$CRED_FILES" ]; then
  ...
  if [ -n "$REAL_CREDS" ]; then
    ...
    echo -e "${RED}[WARN]  HIGH: Potential credential files found!${NC}"
    echo ""
    ...
    cat >> "$REPORT_FILE" <<EOF
[WARN]  HIGH: Potential Credential Files
========================================
...
EOF
    ...
    echo -e "${GREEN}[OK]${NC} No suspicious credential files (only test/example files)"
  fi
else
  echo -e "${GREEN}[OK]${NC} No credential files found"
fi
...
echo -e "${YELLOW}[WARN]${NC}  Scanning code for hardcoded credentials..."
...
if [ -n "$HARDCODED" ]; then
  HIGH_ISSUES=$((HIGH_ISSUES + 1))
  echo -e "${RED}[WARN]  HIGH: Hardcoded credentials found in code!${NC}"
  echo ""
  ...
  cat >> "$REPORT_FILE" <<EOF
[WARN]  HIGH: Hardcoded Credentials in Code
========================================
...
EOF
  ...
  echo -e "${GREEN}[OK]${NC} No hardcoded credentials detected"
fi
...
echo -e "${YELLOW}[WARN]${NC}  Checking configuration files..."
...
if [ -n "$CONFIG_KEYS" ]; then
  ...
  echo -e "${YELLOW}[WARN]  MEDIUM: Potential keys in configuration files${NC}"
  echo ""
  ...
  cat >> "$REPORT_FILE" <<EOF
[WARN]  MEDIUM: Configuration Files with Keys
========================================
...
EOF
  ...
  echo -e "${GREEN}[OK]${NC} No keys in configuration files"
fi
...
echo -e "${YELLOW}[WARN]${NC}  Checking .gitignore for security patterns..."
...
if [ -f ".gitignore" ]; then
  ...
  if ! grep -q "\.env" .gitignore 2>/dev/null; then
    GITIGNORE_OK=false
    MEDIUM_ISSUES=$((MEDIUM_ISSUES + 1))
    echo -e "${YELLOW}[WARN]  MEDIUM: .env not in .gitignore${NC}"
  fi

  if ! grep -q "credentials" .gitignore 2>/dev/null; then
    GITIGNORE_OK=false
    MEDIUM_ISSUES=$((MEDIUM_ISSUES + 1))
    echo -e "${YELLOW}[WARN]  MEDIUM: credentials not in .gitignore${NC}"
  fi

  if [ "$GITIGNORE_OK" = true ]; then
    echo -e "${GREEN}[OK]${NC} .gitignore has security patterns"
  else
    cat >> "$REPORT_FILE" <<EOF
[WARN]  MEDIUM: .gitignore Missing Security Patterns
========================================
...
EOF
  ...
  MEDIUM_ISSUES=$((MEDIUM_ISSUES + 1))
  echo -e "${YELLOW}[WARN]  MEDIUM: No .gitignore file found${NC}"

  cat >> "$REPORT_FILE" <<EOF
[WARN]  MEDIUM: No .gitignore File
========================================
