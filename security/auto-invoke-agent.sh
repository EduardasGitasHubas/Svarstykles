...
if [ ! -f "security/check-triggers.sh" ]; then
  echo -e "${YELLOW}[WARN]${NC} Trigger detection script not found, skipping auto-invoke"
  exit 0
fi
...
case $TRIGGER_LEVEL in
  CRITICAL)
    ...
    echo -e "${RED}[ALERT] CRITICAL TRIGGERS DETECTED${NC}"
    echo ""
    ...
    if [ "$IS_RELEASE" = true ]; then
      ...
      echo -e "${YELLOW}[WARN]${NC}  RELEASE MODE: Automatic deep scan required"
      echo -e "${YELLOW}[WARN]${NC}  This will take 1-2 minutes for thorough analysis"
      echo ""
      ...
      echo -e "${BLUE}[INFO]${NC}  Claude AI will ask if you want to run deep scan"
      echo ""
  ...
  HIGH)
    ...
    echo -e "${YELLOW}[WARN]  HIGH-PRIORITY TRIGGERS DETECTED${NC}"
    echo ""
    echo "Recommendation: $RECOMMENDATION"
    echo ""
    echo -e "${BLUE}[INFO]${NC}  Claude AI will recommend deep scan"
    echo ""
  ...
  MEDIUM)
    ...
    echo -e "${BLUE}[INFO]  Medium-priority conditions detected${NC}"
    echo ""
    echo "Recommendation: $RECOMMENDATION"
    echo ""
  ...
  LOW|NONE)
    ...
    echo -e "${GREEN}[OK]${NC} No significant security triggers detected"
    if [ "$TRIGGER_LEVEL" = "LOW" ]; then
      echo -e "${BLUE}[INFO]${NC} Low-priority conditions noted (informational only)"
    fi
    echo ""
