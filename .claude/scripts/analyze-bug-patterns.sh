...
if [ ! -d "$BUG_REPORTS_DIR" ]; then
  echo -e "${YELLOW}[WARN]${NC} No bug reports directory found"
  echo ""
...
if [ "$REPORT_COUNT" -eq 0 ]; then
  echo -e "${GREEN}[OK]${NC} No bug reports found"
  echo ""
...
echo -e "${BLUE}[INFO]${NC} Found ${BOLD}$REPORT_COUNT${NC} bug report(s)"
echo ""
...
if [ -s "$temp_protocols_rec" ]; then
  ...
  if [ "$max_protocol_count" -gt 1 ]; then
    echo -e "  ${YELLOW}[WARN]${NC} ${BOLD}$max_protocol${NC} has $max_protocol_count failures"
...
if [ -s "$temp_versions_rec" ]; then
  sort "$temp_versions_rec" | uniq -c | while read -r count version; do
    if [ "$count" -ge 3 ]; then
      echo -e "  ${YELLOW}[WARN]${NC} Version ${BOLD}${version}${NC} has $count reports"
...
if [ "$REPORT_COUNT" -ge 5 ]; then
  echo -e "  ${BLUE}[INFO]${NC} ${BOLD}$REPORT_COUNT total reports${NC}"
...
echo -e "${GREEN}[OK]${NC} Summary saved to: ${BOLD}$SUMMARY_FILE${NC}"
echo ""
