...
**Show ONLY:**
- [ERROR] Build errors (if build fails)
- [WARN] Security issues (if credentials found)
- [OK] Commit result: `[OK] Committed (83e637a)`
...
if (security_credentials_found) {
  // SHOW - security issue
  output: `[WARN] Security: ${count} credentials redacted
...
if (forbidden_files_staged) {
  // SHOW - critical error
  output: `[ERROR] Cannot commit: forbidden files staged
...
if (auto_commit === true) {
  ...
  output: `[OK] Committed (${commit_hash})`
  // That's it!
}
...
if (auto_push === true) {
  ...
  output: `[OK] Pushed to origin/${branch}`

  // If auto_pr enabled
  if (auto_pr === true) {
    create_pr_automatically()

    output: `[OK] PR created: ${pr_url}`
  }
}
...
**Output:**
```
[WARN] Security: 2 credentials redacted
...
[19:15:36] [OK] Task 1: Build - success (4.2s)
[19:15:37] [OK] Task 2: Export - 3 sessions (1.1s)
[19:15:38] [OK] Task 3: Security - clean (0.8s)
...
// Git errors
if (git_error) {
  show: `[ERROR] Git error: ${error}
  ${suggested_fix}`
}

// Security critical
if (security_critical) {
  show: `[WARN] Security: credentials found
  Review and confirm: (Y/n)`
}

// Network errors (push failed)
if (push_error) {
  show: `[WARN] Push failed: ${error}
