...
**Show ONLY:**
- [WARN] Crash with uncommitted changes (ask what to do)
- [ERROR] Critical errors (with fix instructions)
- (Optional) [PACKAGE] Update available

**Result:**
- If OK: Show nothing or just `[OK] Ready`
...
if status == "needs_input":
    ...
    # SHOW - ask user
    print(f"[WARN] Previous session crashed\n")
...
# Case 2: Critical error
    ...
    # SHOW - user must fix
    for error in errors:
        print(f"[ERROR] {error['task']} failed")
...
# Case 3: Success
    ...
    if security_task and "SECURITY:redacted" in security_task["result"]:
        ...
        if config.get("cold_start", {}).get("show_security_warnings", False):
            ...
            print(f"[WARN] Security: {count} credentials redacted\n")
...
RESULT=$(py src/framework-core/main.py cold-start)
STATUS=$(echo "$RESULT" | py -c "import sys, json; print(json.load(sys.stdin).get('status', 'error'))")

# Only proceed if status is success
if [ "$STATUS" = "success" ]; then
  # Extract version check result
  VERSION_CHECK=$(echo "$RESULT" | py -c "
import sys, json
...
")
  ...
  if echo "$VERSION_CHECK" | grep -q "UPDATE:available"; then
    ...
    echo "[PACKAGE] Framework update available: v$CURRENT -> v$LATEST"
    ...
    if [ -f "CLAUDE.md.new" ] && [ -f "/tmp/fw-cmd.tar.gz" ]; then
      ...
      if [ "$DOWNLOADED_VERSION" != "$LATEST" ]; then
        echo "[WARN]  Downloaded CLAUDE.md has wrong version (v$DOWNLOADED_VERSION)"
        ...
        echo "   [OK] Version corrected in CLAUDE.md"
      fi
      ...
      echo "[OK] Framework updated to v$LATEST"
      echo ""
      echo "[WARN]  IMPORTANT: Restart this session to use new framework version"
      echo "   Type 'exit' and start new session"
      echo ""
    else
      echo "[WARN]  Update failed - continuing with v$CURRENT"
...
```json
{
  "cold_start": {
    ...
    "show_ready": false,            // Show "[OK] Ready" or not
...
[18:42:16] [OK] Task 1: Migration cleanup (0.2s)
[18:42:17] [OK] Task 2: Crash detection - auto-recovered (0.8s)
[18:42:18] [OK] Task 3: Version check - up to date (1.2s)
[18:42:19] [OK] Task 4: Security cleanup - clean (1.5s)
[18:42:20] [OK] Task 5: Dialog export - 3 sessions (2.1s)
[18:42:20] [OK] Task 6: COMMIT_POLICY - exists (0.1s)
[18:42:20] [OK] Task 7: Git hooks - installed (0.2s)
[18:42:20] [OK] Task 8: Config - exists (0.1s)
[18:42:21] [OK] Task 9: Context loaded (0.5s)
[18:42:21] [OK] Task 10: Session marked active (0.1s)
...
**v2.7.0 (Silent):**
- Output: 0 lines (or 1 line "[OK] Ready")
