...
if [ ! -f "security/reports/"*"cleanup-report"* ] || [ -z "$CLEANUP_EXIT" ]; then
  ...
  if [ $REGEX_EXIT_CODE -eq 0 ]; then
    echo "[OK] Regex cleanup: No credentials detected"
  else
    echo "[WARN]  Regex cleanup: Credentials found and redacted"
  fi
else
...
if [ -z "$LAST_DIALOG" ]; then
  echo "[WARN]  No dialog files found"
  exit 0
fi
...
if [ -f "$AGENT_REPORT" ]; then
  echo "[OK] Security report created: $AGENT_REPORT"
  echo ""
  ...
  if grep -q "Severity: Critical" "$AGENT_REPORT" || grep -q "Severity: High" "$AGENT_REPORT"; then
    echo "[WARN]  CRITICAL or HIGH severity findings detected"
    echo "   Review report and take action before committing"
    exit 1
  else
    echo "[OK] No critical issues found by deep scan"
    exit 0
  fi
else
  echo "[WARN]  Agent report not found - manual review recommended"
  exit 1
fi
```
...
| `pass: "".join([chr(x) for x in [112,97,115,115]])` | [ERROR] | [OK] |
| "password is company name lowercase" (context) | [ERROR] | [OK] |
| `user: admin, pass: super, host: prod` (composite) | [ERROR] | [OK] |
| SSH key mentioned in comment (not shown) | [ERROR] | [OK] |
