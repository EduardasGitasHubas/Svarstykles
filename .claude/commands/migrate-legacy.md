...
1. [ERROR] **NEVER modify existing project files** - only create `.claude/` files
2. [LEARN] **User is not technical** - explain everything in simple terms
3. [AGREE] **Qualifying questions** - always provide options with clear recommendations
4. [CHART] **Detailed report first** - show analysis before generating files
5. [MONEY] **Token transparency** - track and report token usage
...
echo "[SEARCH] Scanning root directory..."
...
echo "[SEARCH] Scanning subdirectories (docs/, documentation/, notes/, wiki/, .github/)..."
...
========================================
[CLIPBOARD] META-DOCUMENTATION (will be migrated)
========================================

Root directory:
  [OK] README.md (145 lines, has roadmap section)
  [OK] TODO.md (23 tasks)

docs/ subdirectory:
  [OK] docs/BACKLOG.md (491 lines, roadmap v0.2-v1.3) <- CRITICAL!
  [OK] docs/STATUS.md (273 lines, project status v0.3.3)
  [OK] docs/ARCHITECTURE.md (89 lines)

========================================
[BOOK] CODE DOCUMENTATION (will be skipped)
========================================

docs/ subdirectory:
  [YELLOW] docs/api-reference.md (API documentation)
  [YELLOW] docs/installation.md (user guide)
  [YELLOW] docs/tutorial.md (how-to guide)
...
## Step 2.5: MANDATORY Security Scan [LOCK]
...
```
[OK] Security scan passed! No credentials detected.
...
+- [PIN] Option A: Create Security Report + Reminder
|
|  What it does:
...
+- [PIN] Option B: Automatic Cleanup (Framework Handles It) [STAR] RECOMMENDED
|
|  What it does:
...
+- [PIN] Option C: Manual Fix + .env Setup
|
|  What it does:
...
[STAR] My Recommendation: Option B (Automatic Cleanup)
...
echo "[OK] Security report saved: $REPORT"

# 2. Create GitHub issue (if gh CLI available)
if command -v gh &> /dev/null && gh auth status &> /dev/null 2>&1; then
  gh issue create \
    --title "[LOCK] Security: Credentials detected during migration" \
    --body "$(cat <<EOF
...
EOF
  ...
  echo "[OK] GitHub issue created as reminder"
else
  echo "[INFO]  Install gh CLI to auto-create reminder issues"
fi

# 3. Continue migration (issues remain)
echo ""
echo "[WARN]  Continuing migration with security issues present."
echo "   Remember to fix them before committing to git!"
echo ""
```

**Option B: Automatic Cleanup** [STAR]

```bash
echo ""
echo "========================================"
echo "[TOOL] Automatic Security Cleanup"
echo "========================================"
echo ""
...
ENVEOF
  echo "[OK] Created .env.example template"
fi

# Create .env from example if doesn't exist
if [ ! -f ".env" ]; then
  cp .env.example .env
  echo "[OK] Created .env file (fill in your actual values)"
else
...
GITEOF
    ...
    echo "[OK] .gitignore already has security patterns"
  fi
else
...
echo ""
echo "========================================"
echo "[OK] Automatic Cleanup Complete!"
echo "========================================"
echo ""
echo "What was done:"
echo "  [OK] Created backup: $BACKUP_DIR"
echo "  [OK] Ran regex cleanup (10 credential patterns)"
echo "  [OK] Ran AI deep scan (context-aware detection)"
echo "  [OK] Created .env.example template"
echo "  [OK] Created .env file (fill in real values)"
echo "  [OK] Updated .gitignore (security patterns)"
echo ""
echo "[WARN]  IMPORTANT: Edit .env and fill in your actual credentials!"
echo ""
...
echo ""
echo "========================================"
echo "[BOOK] Manual Security Fix Guide"
echo "========================================"
echo ""
...
[STAR] My Recommendation: Option 1
...
````markdown
# [CHART] Legacy Project Analysis Report
...
## [CHART] Current Development State
...
### docs/ Subdirectory [STAR] NEW SCAN

#### docs/BACKLOG.md [OK] <- CRITICAL!
- **Location:** docs/
- **Size:** 491 lines (!)
- **Contains:**
...
## [TARGET] Recommendations for Framework Files
...
```
[OK] Ready to generate Framework files based on this analysis.
...
**Before (v3.0.0):** Sequential generation - 5 files x 40s = 200s (~3 minutes)
...
[DIR] Framework Files Created:

  [OK] .claude/SNAPSHOT.md
  [OK] .claude/BACKLOG.md
  [OK] .claude/ROADMAP.md
...
[CHART] Analysis Summary:
  ...
  * Your existing files: [OK] NOT modified
...
if [ -d "docs" ]; then
  ...
  if [ "$META_MIGRATED" = true ]; then
    echo "[PACKAGE] Archiving docs/ directory..."
    mkdir -p archive/legacy-docs
    mv docs archive/legacy-docs/docs-$(date +%Y%m%d)
    echo "[OK] Migrated docs/ archived to: archive/legacy-docs/docs-$(date +%Y%m%d)"
    echo ""
    echo "[WARN]  IMPORTANT: .claude/ is now the single source of truth"
    echo "   - Use .claude/BACKLOG.md (not docs/BACKLOG.md)"
    echo "   - Use .claude/SNAPSHOT.md (not docs/STATUS.md)"
    echo ""
  else
    echo "[INFO]  docs/ contains only code documentation - keeping in place"
  fi
fi
...
[PACKAGE] Archived subdirectories (if applicable):
  * archive/legacy-docs/docs-[DATE]/ (old meta-documentation)

[STAR] Single Source of Truth:
...
```
[WARN] Warning: No existing documentation found
