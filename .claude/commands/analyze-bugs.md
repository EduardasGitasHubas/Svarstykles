...
if [ ! -d "migration" ] || [ ! -f "migration/build-distribution.sh" ]; then
  echo "[WARN]  This command is only available in the framework project"
...
echo ""
echo "========================================"
echo "[CHART] Analyzing Bug Reports"
echo "========================================"
echo ""
...
gh issue list --label "bug-report" --state open --json number,title,body,createdAt,url > /tmp/bug-reports.json 2>/dev/null || {
  echo "[WARN]  Failed to fetch issues. Make sure 'gh' CLI is installed and authenticated."
...
if [ "$REPORT_COUNT" -eq "0" ]; then
  echo "[OK] No open bug reports found"
  echo ""
  exit 0
fi
